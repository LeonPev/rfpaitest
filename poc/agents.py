...
def generate_markdown_output(result: dict) -> str:
    """Generate comprehensive IDF-styled markdown (30+ pages) for leadership delivery."""
    ...
    lines += [
        "---",
        "מסמך: **סודי / RESTRICTED** — לשימוש פנימי בלבד בלבד",
        "---",
        "",
        f"# {title}",
        "",
        "**מסמך תכולת עבודה מעודכן (Statement of Work)**",
        "**גרסה 2026 — משופרת בעזרת בינה מלאכותית**",
        "",
        "---",
        "",
        "## מטליליות המסמך",
        ...
        f"| **מול אישור סופי** | דעיכת קבלת משפחת זמני ואישור משרד הביטחון |",
        "",
        "> ⚠️ **הצהרת סודיות:** מסמך זה וכל נספחיו הינו בבעלות בלעדית של משרד הביטחון. חל איסור מוחלט להעבירו, להעתיקו או לחלוק אותו לצד שלישי ללא אישור בכתב ומראש של משרד הביטחון.",
        "",
        "---",
        "",
    ]
    ...
    lines += [
        ...
        f"### סקירה כללית",
        "",
        f"מסמך זה מתאר את תכולת העבודה המעודכנת והמשופרת לרכש **{domain.lower()}** עבור משרד הביטחון בשנה 2026.",
        "",
        "**משפחת עיקריות:**",
        "- רציפות אספקה מובטחת בתנאי שגרה, עידוד ומצב חירום",
        "- דרישות טכניות ברורות המבוססות על תקנים ישראליים ובינלאומיים",
        "- מנגנון SLA וקנסות מדורג להבטחת ביצועי ספק",
        "- עידוד תחרות דרך חלוקה ל-LOTs ותנאי שקילות",
        "- הגנה על ביטחון המידע במעלות גבוהות",
        "",
        f"**גרסה זו כוללת {len(accepted)} סעיפים עיקריים, טבלאות KPI, מפרטים טכניים ו-{len([q for q in result.get('documents', []) if q.get('type') == 'vendor_qa'])} שאלות הבהרה עם תשובות.**",
        "",
        "---",
        "",
    ]
    ...
    toc_items = [
        ...
        "2. הגדרות ומושגים",
        "3. תקנים ודרישות רגולטוריות",
        "4. LOTs — חלוקה לעידוד תחרות",
        "5. חבילת השירות ודרישות זמן אספקה",
        "6. מפרטים טכניים",
        "7. הבטחת איכות (QA) וביקורות",
        "8. KPIs & SLAs — טבלת עיצומים",
        "9. אנשי קשר וממשקים תפעוליים",
        "10. ניהול מחסן ואחסנה",
        "11. סימון, אריזה ותיוג",
        "12. ביטחון מידע והגנה על סודות",
        "13. מצבי חירום ופרוטוקולים במלחמה",
        "14. תהליכי פיקוח ופתרון סכסוכים",
        "15. עיצומים, פיצויים ודמי ביצוע",
        "16. סעיף מיוחד: ההתקשרות עם תת-קבלנים",
        "נספח א׳: שאלות הבהרה ותשובות מפורטות",
        "נספח ב׳: טבלת עיצומים מפורטת",
        "נספח ג׳: רשימת מעיניות ותקנים",
        "נספח ד׳: טופסי דיווח ובדיקה",
    ]
    ...
    # ── STANDARD SECTION: KPI & SLA TABLE ───────────────────────────────────
    lines += [
        "## 8. KPIs & SLAs — טבלת דרישות ביצוע ועיצומים",
        "",
        "> **[P1 — סעיף חיוני]** כל התקשרות עם משרד הביטחון חייבת להכיל טבלה ברורה של KPI, SLA וקנסות. זה משפחתי להגדרת ציפיות בהיר וליישומן בעדילות ו-בחוקיות.",
        "> *סיבה: הגדרת SLA בעדילות חוקית ודיוק מודד, תחתון חזק לעדיפות השוטפת של משרד הביטחון.*",
        "",
        "### א. טבלת KPI עיקריים",
        "",
        "| # | KPI | יעד | יחידה | שכיחות בדיקה | קנס (%) |",
        "|---|-----|------|------|------------|--------|",
        "| 1 | זמן אספקה שגרה | 24 שעות | שעות | שבועי | 0.5 |",
        "| 2 | זמן אספקה דחוף | 4 שעות | שעות | יומי | 2.0 |",
        "| 3 | זמן אספקה חירום | 2 שעות | שעות | בזמן אמת | 5.0 |",
        "| 4 | אחוז דיוק מסמכים | 99.5% | % | לכל הזמנה | 1.0 |",
        "| 5 | אחוז איכות טכנית | 98% | % | דגימות ממשמרות | 2.0 |",
        "| 6 | עדכון מעמדות בשיטה | עד 1 שעה | שעות | בזמן אמת | 0.3 |",
        "",
        "### ב. מנגנון עיצומים (Penalty Schedule)",
        "",
        "| אירוע | אחוז עיצום | משך היכול | הערות |",
        "|-------|-----------|----------|-------|",
        "| איחור 24-48 שעות | 0.5% מ-SO | עד 7 ימים | עם הודעה ראשונה |",
        "| איחור 48-72 שעות | 1.5% מ-SO | שבוע | דרישה להסבר כתוב |",
        "| איחור מעל 72 שעות | 3.0% מ-SO | 30 ימים | סף הפסקת אספקה |",
        "| טעות בנתונים/מאפיינים | 2% מ-SO | הזמנה | בכל אירוע |",
        "| דליפה סביבתית | 10% מ-SO + ביקורת | חודש | בנוסף לעלות תיקון |",
        "| הפרת סודיות מידע | עיצום עד 50% מ-SO | 3 חודשים | דיווח למשטרה |",
        "",
        "---",
        "",
    ]

    # ── STANDARD SECTION: TECHNICAL SPECIFICATIONS ──────────────────────────
    lines += [
        "## 9. מפרטים טכניים ודרישות סטנדרטיות",
        "",
        "> **[P1 — סעיף קריטי]** כל פריט/שירות חייב להיות מעוגן לתקן ספציפי (ISO, EN, ASTM, מיל-סטנדרד). ללא זה, אין אכיפה טכנית ברורה.",
        "",
        "### א. תקנים חלים",
        "",
        "- **ISO 9001:2015** — ניהול איכות",
        "- **ISO 14001:2015** — ניהול סביבה",
        "- **ISO 45001:2018** — בטיחות ובריאות בעבודה",
        "- **ISO 27001:2013** — אבטחת מידע",
        "- **EN/IEC 62304** (אם רלוונטי לציוד רפואי)",
        "- **ADR 2025** (להובלת חומרים מסוכנים)",
        "",
        "### ב. מאפיינים חיוביים ודרישות ביצוע",
        "",
        "| פריט | דרישה טכנית | יחידה | סף קבול | שיטת בדיקה |",
        "|-------|------------|------|--------|-----------|",
        "| דוגמה: זמן התגובה | עד 4 שעות | שעות | 4.0 | מערכת SRM |",
        "| דוגמה: שיעור זמינות | 98.5% | % | 98.5 | דיווח חודשי |",
        "| דוגמה: תאימות תקנים | 100% | % | 100 | בדיקה ראשונית + דגימות |",
        "",
        "---",
        "",
    ]

    # ── STANDARD SECTION: QUALITY ASSURANCE ─────────────────────────────────
    lines += [
        "## 10. הבטחת איכות (QA) וביקורות תקופתיות",
        "",
        "### א. בדיקות ראשוניות (Type Testing)",
        "",
        "- דגם מאשר (Sample Approval) לפני הזמנה מלאה",
        "- תעודות תאימות (CoC) מצד ג' מוסמך",
        "- בדיקות למעבדה מוסמכת עבור כל אצווה",
        "",
        "### ב. ביקורות תקופתיות ובדיקות שדה",
        "",
        "- ביקורות רבעוניות על אתר הספק (Audit)",
        "- ביקורות על קבלנים וספקי משנה",
        "- דגימות מקרית מנקודות האספקה",
        "- דיווחים חודשיים על מדדי איכות",
        "",
        "---",
        "",
    ]

    # ── STANDARD SECTION: SECURITY & CONFIDENTIALITY ─────────────────────────
    lines += [
        "## 11. ביטחון מידע והגנה על סודות",
        "",
        "> **[P1 — קריטי]** משרד הביטחון נדרש לשמור על מידע מסווג ברמה גבוהה. כל הפרה תוביל לעיצומים חמורים וסנקציות משפטיות.",
        "",
        "### א. דרישות אבטחה חיוביות",
        "",
        "- הצפנה כל מידע בעתיד (AES-256 או שווה)",
        "- בקרת גישה רב-שלבית (MFA) על כל מערכות",
        "- ביקורות סייבר חצי-שנתיות ממטרד הביטחון",
        "- דיווח מיידי על כל חשיפה או הפרה (within 24 hours)",
        "- חוזה סודיות חתום עם כלל העובדים",
        "",
        "### ב. עיצומים על הפרת סודיות",
        "",
        "- עיצום של עד 50% מעלות החוזה החודשית",
        "- דיווח למשטרה על כל הפרה חמורה",
        "- ביטול מיידי של ההתקשרות ללא מקדמה",
        "",
        "---",
        "",
    ]

    # ── STANDARD SECTION: EMERGENCY & WAR PROTOCOLS ──────────────────────────
    lines += [
        "## 12. פרוטוקולים במצבי חירום ומלחמה",
        "",
        "### א. רמות תגובה",
        "",
        "| מצב | זמן תגובה | דרישות תוספות |",
        "|-----|----------|---------------|",
        "| שגרה | עד 24 שעות | אחסון תקני, סדר ראשון-הוא-מהיר-יוצא |",
        "| תיעדוף (High Priority) | עד 4 שעות | קביעת זמן מחזור מודדת, תוכנת בדיקה מהירה |",
        "| חירום (Emergency) | עד 2 שעות | צוות חירום ייעודי, כלל המשאבים |",
        "| מלחמה (War) | עד 1 שעה | משמרות כפולות, מוקדים חלופיים |",
        "",
        "### ב. תנאים נדרשים בתקופת חירום",
        "",
        "- צוות כוננות 24/7 מתוך הספק",
        "- מנהל תפעול בכיר זמין ללא הפסקה",
        "- קו תקשורת ישיר למחלקת הרכש הטקטית",
        "- רשאות מיידית לתנוא דרישות תפעוליות",
        "- דיווח סטטוס שעתי למרכז החירום",
        "",
        "---",
        "",
    ]
    ...
    if qa_items:
        lines += [
            "## נספח א׳ — שאלות הבהרה מפורטות ותשובות מחייבות",
            "",
            "| # | שאלה | תשובה | קטגוריה | שדרוג |",
            "|---|------|--------|----------|--------|",
        ]
    ...
    # ── QUALITY CHECKLIST ───────────────────────────────────────────────────
    lines += [
        "## נספח ב׳ — רשימת בדיקה לאישור סופי (Quality Gate Checklist)",
        "",
        "לפני פרסום סופי של המסמך, בדוק את הבאים:",
        "",
        "- [ ] כל סעיף בעל דרישה טכנית מעוגנת לתקן (ISO, EN, ASTM, וכו.)",
        "- [ ] טבלת KPI וSLA ברורה עם קנסות מדורגים",
        "- [ ] סעיף סודיות ואבטחת מידע חתום",
        "- [ ] פרוטוקולי חירום ומלחמה מפורטים",
        "- [ ] LOTs מוגדרים בצורה שמעודדת תחרות",
        "- [ ] שאלות הבהרה וקטגוריות בעלות השפעה ברורה",
        "- [ ] כל שינוי (P1/P2) מסומן וממוצא",
        "- [ ] אנשי קשר ו-POC מעודכנים",
        "- [ ] תקני עיצומים ובחוק משפטי",
        "",
        "---",
        "",
    ]
    ...
    lines += [
        "## הערה סופית",
        "",
        f"מסמך זה הופק אוטומטית ע\"י מערכת **AI לרכש חכם** ביום {today}.",
        "",
        "**⚠️ מיועד לעיון חזקה לפני אישור סופי בעזרת גורמים משפטיים וטכניים של משרד הביטחון.**",
        "",
        "כל הערה, תיקון או בקשה לתיקון יש להעביר לרכז המיזם בחשכ\"ל.",
        "",
    ]
